...
export username=$(oc exec -c elasticsearch -n $AIOPS_NAMESPACE -it aiops-ibm-elasticsearch-es-server-all -- bash -c 'cat /usr/share/elasticsearch/config/user/username')	
export password=$(oc exec -c elasticsearch -n $AIOPS_NAMESPACE -it aiops-ibm-elasticsearch-es-server-all -- bash -c 'cat /usr/share/elasticsearch/config/user/password')	
