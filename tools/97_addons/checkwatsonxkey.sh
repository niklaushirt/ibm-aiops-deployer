...
echo ""
echo "retrieval for API Key $API_KEY"
echo ""
...
if echo "$RESPONSE" | jq -e '.errorMessage' > /dev/null 2>&1; then
  echo ""
  echo "Error returned from IAM. Exiting."
  echo ""
  exit 1
fi

read -p "Press enter to continue retrieving associated project IDs"

IAM_TOKEN=$(echo "$RESPONSE" | jq -r .access_token)

curl -s \
-H "Authorization: Bearer $IAM_TOKEN" \
"https://api.dataplatform.cloud.ibm.com/v2/projects?limit=100" | jq -r
