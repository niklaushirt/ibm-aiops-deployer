...
if [[ $SKIP_CONFIRM != "true" ]]; then
  ...
  log " "
   if [[ $IA_ENABLED == "true" ]]; then
      read -p "You have selected IA_ENABLED to be true. This indicates you would like to leave Infrastructure Automation running. Are you sure you would like to proceed?  Y or y to continue: " -n 1 -r
      log " "
      if [[ ! $REPLY =~ ^[Yy]$ ]]; then
         log $INFO "Cancelling uninstall of IBM Cloud Pak for AIOps."
         exit 0
